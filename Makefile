...
generate: ## Generate CRDs, DeepCopy methods, and RBAC.
	...
	go generate ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...
...
fmt: ## Run go fmt against code.
	go fmt ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...

.PHONY: vet
vet: ## Run go vet against code.
	go vet ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...

.PHONY: test
test: generate fmt vet envtest ## Run tests.
	go test -v ./pkg/client/...
	go test -v ./pkg/proxy/...
	go test -v ./pkg/termite/...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test -v ./pkg/operator/...

.PHONY: lint
lint: ## Run linters against code (modernize, golangci-lint, testifylint).
	go run golang.org/x/tools/gopls/internal/analysis/modernize/cmd/modernize@latest -fix -test ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...
	go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@latest run --fix ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...
	go run github.com/Antonboom/testifylint@latest --fix ./pkg/client/... ./pkg/operator/... ./pkg/proxy/... ./pkg/termite/...
